...
columns_to_incl <- c('CAS.Number', 'SMILES', 'Molecular.formula', 'Predefined.substance.type', 'Additional.Ids', 'Identity', 'Database', 'Effect', 'Endpoint', 'Comments',
                     'Endpoint.comments', 'Title', 'Author', 'Year', 'Publication.year', 'URL', 'DOI', "Substance.Test.material.equality", 'Test.method', 'Test.method.comments', 
                     'Test.guideline', 'Data.quality', 'Qualifier', 'Measurement', 'Reference.source', 'Control.type', 'Organism.lifestage', 'Experimental.design', 
                     'Principles.of.method.if.other.than.guideline', 'Test.type', 'Media.type', 'Water.type', 'Subhabitat','Test.organisms..species.', 'Duration.MeanValue', 'Duration.Qualifier',
...
HESTIA_HC20_DB_raw <- rbind(
  read.csv("data/2022-11-07/QSAR_Aquatic_ALL_TOX_2022-11-07_with_metadata[1-4k].csv",header = T, sep = "\t", na.strings = "", fileEncoding = "UTF-16LE",  stringsAsFactors = FALSE) %>% 
    select(all_of(columns_to_incl)),
  read.csv("data/2022-11-07/QSAR_Aquatic_ALL_TOX_2022-11-07_with_metadata[4k-8k].csv",header = T, sep = "\t", na.strings = "", fileEncoding = "UTF-16LE",  stringsAsFactors = FALSE) %>% 
    select(all_of(columns_to_incl)),
  read.csv("data/2022-11-07/QSAR_Aquatic_ALL_TOX_2022-11-07_with_metadata[8k-12k].csv",header = T, sep = "\t", na.strings = "", fileEncoding = "UTF-16LE",  stringsAsFactors = FALSE) %>% 
    select(all_of(columns_to_incl)),
  read.csv("data/2022-11-07/QSAR_Aquatic_ALL_TOX_2022-11-07_with_metadata[12k-16k].csv",header = T, sep = "\t", na.strings = "", fileEncoding = "UTF-16LE",  stringsAsFactors = FALSE) %>% 
    select(all_of(columns_to_incl)),
  read.csv("data/2022-11-07/QSAR_Aquatic_ALL_TOX_2022-11-07_with_metadata[16k-end].csv",header = T, sep = "\t", na.strings = "", fileEncoding = "UTF-16LE",  stringsAsFactors = FALSE) %>% 
    select(all_of(columns_to_incl))
) %>% 
...
write.csv(HESTIA_HC20_DB_raw, "data/HESTIA_DB_pre_filtered.csv")

# Creating a more condensed data file for the toxicological data wrangle, to reduce runtime
HESTIA_HC20_DB_raw_toxdata <- HESTIA_HC20_DB_raw[, 1:48]
write.csv(HESTIA_HC20_DB_raw_toxdata, "data/HESTIA_HC20_DB_raw_toxdata.csv")
