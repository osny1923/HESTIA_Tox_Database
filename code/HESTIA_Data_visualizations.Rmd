  ...
  mutate(
   ...
   group_by(CAS.Number) %>% 
    summarise(
              ...
              #median_sp_Cov = median(sp_Cov), # Coefficient of variation for all data points per species group) 
              n = n(),
              n_Species = n_distinct(Species[!is.na(EC10eq)]),
              n_Taxonomy.Group_all = n_distinct(Taxonomy.Group[!is.na(meanEC10eq)]),
              ...
              left_join(., # Adding the mean and median CoV per species (for each substance)
                        HESTIA_BASE_EnviroTox_FILL %>%
                          ...
                          by = "CAS.Number")
##############
#Problems here: Warning: invalid factor level, NA generated
##############
## Which one is a factor??
```
...
group_by(CAS.Number, Taxonomy.Group, Species) %>% 
  ...
  ungroup() %>%
group_by(CAS.Number) %>% 
  summarise(median_sp_CoV = median(sp_CoV, na.rm = TRUE),
            n_sp_median = sum(!is.na(sp_CoV)),
...
## Looking at the EC10eq data range of distribution 
...
EC10eq_range <- HESTIA_BASE_EnviroTox_FILL %>% 
    filter(CAS.Number %in% HC20_dataset$CAS.Number) %>% 
    group_by(CAS.Number) %>% 
    summarise(sp_CoV = goeveg::cv(EC10eq, na.rm = TRUE),
                                      n_sp = n(),
...
max_HC20 <- HESTIA_envirotox_EC10eq_second_avg %>% 
    slice_max(CRF_best, n = 50) %>% 
    arrange(desc(CRF_best)) %>% 
    ungroup() %>% 
    select(CAS.Number, CRF_best, CoV_sp_mean_EC10eq) %>% 
    mutate(Group = "max", 
           CoV_sp_mean_EC10eq = round(CoV_sp_mean_EC10eq, 3),
           Order_col = seq(1,50)) %>% 
    left_join(x = ., 
              y = Q_dat_averages %>% 
...
min_HC20 <- HESTIA_envirotox_EC10eq_second_avg %>% 
    slice_min(CRF_best, n = 50) %>% 
    arrange(desc(CRF_best)) %>% 
    ungroup() %>% 
    select(CAS.Number, CRF_best, CoV_sp_mean_EC10eq) %>% 
    mutate(Group = "min", 
...
p_max <- ggplot(HESTIA_BASE_EnviroTox_FILL %>%
                filter(CAS.Number %in% max_HC20$CAS.Number) %>% # <-  can wiggle here to present both high and low data, only low data or only high data
                  mutate(Group = case_when(CAS.Number %in% min_HC20$CAS.Number ~"min",
...
p_min <- ggplot(HESTIA_BASE_EnviroTox_FILL %>%
...
Plotting CoV for all substances in falling magnitude
...
Plotting toxicity according to largest CoV for top 50 CoV substances based on chronic data only
...
max_CoV_chronic <- Q_dat_averages %>% 
    ...
    #ungroup() %>% 
    select(CAS.Number, CoV_logec10eq_chronic, meanEC10eq, medianEC10eq, n) %>% 
    mutate(Group = "max",
           CoV_logec10eq_chronic = round(CoV_logec10eq_chronic, 2),
...
max50CoV_falling_CoV <- ggplot(HESTIA_BASE_EnviroTox_FILL %>%
