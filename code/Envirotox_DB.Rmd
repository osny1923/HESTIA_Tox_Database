  ...
  # Harmonizing some Class annotations and "correcting" spelling.
              mutate(
                ...
                Class = case_when(Species == "Tubifex tubifex" ~ "Clitellata", 
                                    TRUE ~ Class)) %>% 
              left_join(x = .,
                        y = read.xlsx("../results/Final_Taxonomy_dataset.xlsx", sheetName = "Data1", header = T) %>% 
...
EnviroTox_toxDB <- readxl::read_xlsx(path = "../data/Envirotox_2023-01-10/envirotox_DB_20230110081208.xlsx", 
                                     sheet = "test", 
                                     col_names = T, 
                                     col_types = c(
                                       "skip", 
                                        rep("text", times = 4), 
                                        "guess", rep("text", times = 4), 
                                        "guess", 
                                        "guess", 
                                        "logical", 
                                        rep("text", times = 3), 
...
write.csv(EnviroTox_toxDB_EC10eq, "../data/EnviroTox_toxDB_EC10eq.csv")
