...
I using R package `taxize` and the species list in supp.mat TS4 from Saouter et al., 2019: 
"Saouter, E., Biganzoli, F., Pant, R., Sala, S., and Versteeg, D. (2019) Using REACH for the EU Environmental Footprint: Building a Usable Ecotoxicity Database, Part I. Integr Environ Assess Manag 15: 783–795."
## Making a Taxonomic classification list for all species

```{r, include = F, eval=FALSE}
Species_list <- HESTIA_HC20_DB_prefil %>% 
    distinct(Test.organisms..species.) 
    

#NCBI_Taxon_query <- rbind(classification(get_uid(Species_list$Test.organisms..species.), 
 #                      db = 'ncbi',
  #                     rows = 1,
   #                    verbose = TRUE))
saveRDS(NCBI_Taxon_query, file = "NCBI_Taxon_query.txt")
NCBI_Taxon_query <- readRDS("NCBI_Taxon_query.txt")
NCBI_Taxon_query_matrix <- NCBI_Taxon_query %>%
      select(-id) %>% 
   pivot_wider(names_from = rank, values_from = name)

QSAR_phylum <- NCBI_Taxon_query %>%
                      filter(rank == "phylum") 
QSAR_genus <- NCBI_Taxon_query %>%
                      filter(rank == "genus") 
QSAR_order <- NCBI_Taxon_query %>%
                      filter(rank == "order") 
QSAR_class <- NCBI_Taxon_query %>%
                      filter(rank == "class") 
QSAR_species <- NCBI_Taxon_query %>%
                      filter(rank == "species") 

                            
First_taxonomy <- QSAR_phylum %>% 
                left_join(QSAR_genus, by = "query") %>% 
...
second_taxonomy <- rbind(
        First_taxonomy,
        Species_list %>% 
          filter(!Test.organisms..species. %in% First_taxonomy$Species,
...
# For the 480 missing taxonomic descriptors, where names are old or common or just bad, i do another search.
bad_names <- HESTIA_HC20_DB %>% 
  filter(!Test.organisms..species. %in% second_taxonomy$Species,
         grepl(" ", Test.organisms..species.)) %>% 
  select(Test.organisms..species.) %>% 
  distinct(Test.organisms..species.)
  
# Taxonomy Query (UID - NCBI database identifier)
# good_names <- get_uid(bad_names$Test.organisms..species., modifier = c("All names", "Rank", "Division", "Synonym"))
# write_rds(good_names, "Taxonomy_UID.txt")
good_names <-readRDS("Taxonomy_UID.txt")


bad_names$uid <- good_names

# NCBI_bad_names <- classification(id = good_names, db = "ncbi")
 #write_rds(NCBI_bad_names, "Taxonomy_classification_UID.txt")
NCBI_bad_names <- readRDS("Taxonomy_classification_UID.txt")

NCBI_bad_names_rbind <- rbind(NCBI_bad_names)
pivot_wider(NCBI_bad_names_rbind, names_from = c(id, rank), values_from = name)
  # Binding rows with each taxonomic rank and then compiling into one df
NCBI_phylum <- NCBI_bad_names_rbind %>%
                      filter(rank == "phylum") 
NCBI_genus <- NCBI_bad_names_rbind %>%
                      filter(rank == "genus") 
NCBI_order <- NCBI_bad_names_rbind %>%
                      filter(rank == "order") 
NCBI_class <- NCBI_bad_names_rbind %>%
                      filter(rank == "class") 
NCBI_species <- NCBI_bad_names_rbind %>%
                      filter(rank == "species") 

Bad_names_second <- NCBI_phylum %>% 
...
third_taxonomy <- rbind(second_taxonomy %>% 
      mutate(Species_update = NA_character_),
 
      left_join(x = bad_names %>% 
              rename(Species = Test.organisms..species.,
                     query = uid) %>% 
...
fifth_taxonomy <- rbind(fourth_taxonomy %>% 
                          filter(!Species %in% merge_names_missing$Test.organisms..species.),
                        merge_names_missing %>% 
                                  mutate(Source = "NCBI",
                        ...
                        mutate(Phylum = case_when(Class == "Dinophyceae" ~ "Myzozoa", # algae
...
Final_taxonomy <- seventh_taxonomy %>% 
 mutate(Taxonomy.Group = case_when( Phylum == "Chordata" & Class %in% c("Amphibia", "Amphibian") ~ "Amphibian",
