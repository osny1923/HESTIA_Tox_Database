...
Ser nu att de är missvisande siffor i datasetet. 
Det kanske FINNS 29 datapunkter, från 9 arter, men ett medianvärde från 1 art kan användas till att beräkna medianen av CoV, bara på en art. detta måste jag följa för att kunna visa hur många arter som ingår i medianberäkningen. med någon slags sum(!is.na())
...
#####
# investigating an outlier with a very high median CoV: CAS.number 98079-51-7.
#####
# So, what's the outlier here? with a very high median CoV? it's CAS.number 98079-51-7
Q_dat_averages %>% 
    filter(CAS.Number %in% QSe_over_1.77$CAS.Number) %>% 
    arrange(desc(median_sp_CoV)) %>% 
    slice(1)

# looking at the data spread for CAS 98079-51-7
ggplot(Q_dat %>% 
    filter(CAS.Number == "98079-51-7"),
    aes(x=Species, y=EC10eq, color = Title)) +
  geom_point() +
  scale_y_log10() +
  theme(legend.position = "top",
        legend.text = element_text(size = 6))
# so for CAS number 98079-51-7, the carotenoid content measurement is skewing the data set by a lot. Removing it, CoV goes from 2.43 -> 1.001485. 
Q_dat %>% 
    filter(CAS.Number == "98079-51-7", 
           Measurement != "Carotenoid content") %>%
    group_by(Species) %>% 
    summarise(CoV_bajs = goeveg::cv(EC10eq, na.rm = TRUE))

# the measurement carotenoid content occurs at 36 places in the dataset. 10 algae and 26 plant data points.
Q_dat %>% 
    #filter(#CAS.Number == "98079-51-7", 
     #      Measurement == "Carotenoid content") %>% 
    count(Effect, Measurement, sort = T)
...
HESTIA_HC20_DB_endpoint_conversions %>% 
    group_by(Endpoint_conv, Endpoint) %>% 
    summarise(n = n())
...
write.xlsx(x = 
           ...
           file = "summary_Taxonomy_Endpoint.xlsx",
...
write.xlsx(count_overview, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "SP_Groups")

Group_quality_summary <- HESTIA_envirotox_cfs %>% 
    count(Group, QSe_source)
write.xlsx(Group_quality_summary, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "Group_quality_summary", append = T)

Group_summary <- HESTIA_envirotox_cfs %>% 
    count(Group)
write.xlsx(Group_summary, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "Group_summary", append = T)

QSe_summary <- HESTIA_envirotox_cfs %>% 
    count(QSe_source)
write.xlsx(QSe_summary, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "CRF_Quality Overview", append = T)
...
write.xlsx(HC50_summary, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "HC50_summary", append = T)

FULL_DB_substances <- NEW_PHYSCHEM %>% 
    filter(CAS.Number %in% HESTIA_BASE_EnviroTox_FILL$CAS.Number) %>% 
    count(Group, sort = T) 
write.xlsx(FULL_DB_substances, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "full_DB_substance_type_count", append = T)
...
HESTIA_DB_overview <- Q_dat %>% 
    ...
    ungroup() %>% 
    pivot_wider(names_from = c("AcuteChronic", "Endpoint_conv"), values_from = n)
write.xlsx(HESTIA_DB_overview, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "HESTIA_DB_endpoint_count", append = T)
...
Envirotox_DB_overview <- HESTIA_BASE_EnviroTox_FILL %>% 
    ...
    ungroup() %>% 
    pivot_wider(names_from = c("AcuteChronic", "Endpoint_conv"), values_from = n)
write.xlsx(Envirotox_DB_overview, "HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "Envirotox_DB_endpoint_count", append = T)
...
HESTIA_HC20_DB_raw %>% 
        count(Endpoint) %>% 
write.xlsx("HESTIA_Envirotox_Data_count_overview.xlsx", sheetName = "HESTIA_RAW_Endpoint_Count", append = T)
